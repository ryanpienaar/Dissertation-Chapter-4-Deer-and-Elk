...
Forest <- as.data.frame(substr(rownames(deerocc),5,5))
Fire <- as.data.frame(substr(rownames(deerocc),7,7))

covs <- cbind(Forest, Fire)

deerocc <- unmarkedFrameOccu(y = deerocc, siteCovs = covs)
summary(deerocc)
deerocc
...
deer1 <- occu(~1~1, data =  deerocc)
deer1

deer1.psi <- backTransform(deer1, type = "state")
deer1.psi

deer1.p <- backTransform(deer1, type = "det")
deer1.p
